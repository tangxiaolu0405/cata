...
func (t *Terminal) handleOutputChar(r rune) {
	...
	// CJK 等宽字符占 2 列：在下一格预留空白续格，避免字形与后续字符重叠。
	// （原始实现一律前进 1 列，宽字符字形溢出到下一格造成文字重叠。）
	if width := runewidth.RuneWidth(r); width >= 2 && col+1 < int(t.config.Columns) {
		t.content.SetCell(row, col+1, widget.TextGridCell{Style: cellStyle}) // Rune 0 -> 渲染为空白
		t.cursorCol += 2
	} else {
		t.cursorCol++
	}
...
func handleOutputBackspace(t *Terminal) {
	...
	col := t.cursorCol - 1
	// 光标落在宽字符的续格（空白格）上时，再退一格回到宽字符起点。
	if col > 0 && col < len(row.Cells) && row.Cells[col].Rune == 0 &&
		runewidth.RuneWidth(row.Cells[col-1].Rune) >= 2 {
		col--
	}
