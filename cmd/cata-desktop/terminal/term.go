...
type Terminal struct {
	...
	// done 随 close() 关闭，用于停止 PWD 轮询等后台 goroutine（防泄漏）。
	done     chan struct{}
	doneOnce sync.Once
	// lastKeyTime 最近一次按键时间（实例级，跨终端不串扰）。由主线程写、
	// PWD watcher goroutine 读，用原子 int64 纳秒时间戳。
	lastKeyTime atomic.Int64
	// procMu 保护 cmd.ProcessState 与 pty 的跨 goroutine 写读（Windows wait goroutine
	// 写、ExitCode/updatePTYSize 读），消除数据竞争。
	procMu sync.Mutex
	// printMu 保护 printData 的跨 goroutine 写读（reader goroutine 写、
	// escapePrinterMode 经 fyne.Do 在主线程读）。
	printMu sync.Mutex
}
...
// waitForValidSize 等待 Columns > 2（有有效布局区域）；超时或 done 关闭返回错误。
func (t *Terminal) waitForValidSize(timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	for t.config.Columns <= 2 {
		select {
		case <-t.done:
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("terminal: no valid layout size within %s", timeout)
		}
...
// ForceClose 强制关闭 PTY（^D 未能让 shell 退出时兜底）。
// 关闭 PTY 让内核向终端前台进程组发 SIGHUP，避免遗留孤儿进程；
// 同时关闭 done 停止后台轮询 goroutine。
func (t *Terminal) ForceClose() {
...
// markKeyActivity 记录最近按键时间（实例级原子，供 PWD 轮询判断用户是否在打字）。
func (t *Terminal) markKeyActivity() {
	t.lastKeyTime.Store(time.Now().UnixNano())
}

// recentKeyActivity 最近 0.5s 内是否有按键（PWD 轮询抑制 cd 命令路径跳变）。
func (t *Terminal) recentKeyActivity() bool {
	last := time.Unix(0, t.lastKeyTime.Load())
	return time.Since(last).Seconds() <= 0.5
}
...
func (t *Terminal) run() {
	...
	for {
		...
		if err != nil {
			...
			// 非预期读错误一律 break（EOF / broken pipe / closed），不再继续循环，
			// 避免 "bad file descriptor" 等错误时无限快速重读烧 CPU。
...
// isExpectedPTYReadError 判断是否为终端正常退出的读错误（EOF/broken pipe/closed）。
func isExpectedPTYReadError(err error) bool {
	...
	if pe, ok := err.(*os.PathError); ok {
		msg := pe.Err.Error()
		return msg == "input/output error" || msg == "file already closed"
	}
	return false
}
...
func (t *Terminal) RunLocalShell() error {
	...
	if err := t.waitForValidSize(5 * time.Second); err != nil {
		return err
	}
	err := t.open()
	if err != nil {
