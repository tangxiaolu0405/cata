#!/usr/bin/env bash
# install_cata_macos.sh — 下载 GitHub Release、解压并配置 PATH（macOS）
#
# 用法:
#   curl -fsSL https://raw.githubusercontent.com/tangxiaolu0405/cata/main/install_cata_macos.sh | bash
#   CATA_VERSION=v0.1.9 ./install_cata_macos.sh
#
# 环境变量:
...
detect_artifact() {
  ...
  arch="$(uname -m)"
  case "$arch" in
    arm64|aarch64) ARTIFACT="cata-darwin-arm64" ;;
    x86_64) ARTIFACT="cata-darwin-amd64" ;;
